...
複数の画像を1つの画像環境に含めたいときがあるかもしれません. Sub-figures\index{figure!sub-figures} を使って, 複数の画像を1つの環境に配置しそれぞれに副題を与えることで, サブ図 (複数の図) をまとめる\index{図!複数の図をまとめる}ことができます.

複数の図をまとめるには LaTeX パッケージの **subfig**\index{LaTeX package!subfig} が必要です. `pdf_document` 出力の YAML オプションの `extra_dependencies`\index{output option!extra\_dependencies} で読み込ませることができます. 例としては.
...
- `fig.ncol`\index{チャンクオプション!fig.ncol}: サブ図の列の数です. デフォルトでは全てのグラフが単一の行に並べられます. これを使って複数の行に分けられます.
